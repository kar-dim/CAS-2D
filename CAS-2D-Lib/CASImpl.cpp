...
CASImpl::CASImpl()
    : casOutputBuffer(nullptr), hostOutputBuffer(nullptr), pinnedHostOutputBuffer(nullptr), hasAlpha(false), rows(0), cols(0), totalBytes(0), context(cl_utils::createOpenCLContext(queue, device)),
      casProgram(cl_utils::buildCasKernel(context, device)) {}

// destructor, unmaps pinned memory, everything else is handled with RAII (cl::Buffers etc)
CASImpl::~CASImpl() { destroyPinnedMemory(); }
...
const unsigned char* CASImpl::sharpenImage(const int casMode, const float sharpenStrength, const float contrastAdaption) {
    // execute kernel
    try {
        queue.enqueueNDRangeKernel(cl_utils::KernelBuilder(casProgram, "cas").args(tex, (int)hasAlpha, casMode, sharpenStrength, contrastAdaption, casOutputBuffer).build(), cl::NDRange(),
                                   cl::NDRange(texKernelDims.cols, texKernelDims.rows), cl::NDRange(16, 16));
        queue.finish();
        // copy from GPU to HOST
        queue.enqueueReadBuffer(casOutputBuffer, CL_TRUE, 0, totalBytes, hostOutputBuffer);
        return hostOutputBuffer;
    } catch (const cl::Error&) { return nullptr; }
}
